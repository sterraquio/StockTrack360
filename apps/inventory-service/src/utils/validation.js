...
export const parseNullableDate = (value, fieldName) => {
  if (value === undefined || value === null || value === "") {
    return null;
  }

  if (typeof value !== "string" || !/^\d{4}-\d{2}-\d{2}$/.test(value)) {
    throw validationError("Fecha invalida.", { field: fieldName });
  }
...
export const parseNullableNonNegativeInteger = (value, fieldName) => {
  if (value === undefined || value === null || value === "") {
    return null;
  }

  if (!Number.isInteger(value) || value < 0) {
    throw validationError("El valor numerico no es valido.", {
      field: fieldName,
    });
  }
